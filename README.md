...
│   ├── 环境/                   conda_list + 环境验证
│   ├── 推理输出/               4模型推理输出(按问题分组)
│   ├── 架构分析/               架构报告 + 4模型对比 + 参数统计
│   ├── 分词实验/               tokenizer_experiments.ipynb
...
所有脚本**从 `Week1/` 根目录运行**(脚本内用 `./models/...` 相对路径):
...
- `llm_exp`(py3.10):推理/分析,MPS 版 PyTorch + transformers
- `llama_factory`(py3.11):LLaMA-Factory 训练
- 详见 `docs/任务与约束.md`
